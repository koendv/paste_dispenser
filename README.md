...
On a [zapta](https://www.thingiverse.com/thing:1119914), current drawn is typically XXX when idle, XXX with the stepper running at speed. 
...
For practice fill a syringe with a small amount (approx. 1 ml) of toothpaste. Print a pcb at scale 1:1 in black and white on paper, like this XXX, and practice depositing toothpaste until you are comfortable with the solder paste dispenser.
...
There is no capacitor between DTR and reset. If you want to develop software for this board you have three options: 
...
Generate an Intel Hex file with Sketch -> Export Compiled Binary. This creates two .hex files in the project directory, `arduino.ino.eightanaloginputs.hex` and `arduino.ino.with_bootloader.eightanaloginputs.hex`. 
Now connect a 5V Arduino, e.g. an Arduino Uno to your system. Compile and upload the File -> Examples -> ArduinoISP sketch to the Uno. Connect the Arduino Uno to the solder paste dispenser board as follows:
...
`./bin/avrdude -C ./etc/avrdude.conf  -p m328p -P /dev/cu.usbmodem1d11 -c avrisp -b 19200 -U flash:w:/Users/koen/paste_injector/arduino/arduino.ino.with_bootloader.eightanaloginputs.hex`
where `/dev/cu.usbmodem1d11` is the usb port of your Arduino Uno (On linux probably `/dev/ttyACM0`). Flashing should take approx. 20 seconds, and ends with "`xxx bytes of flash verified`" "`Fuses OK`". This finishes writing bootloader and sketch to the solder paste dispenser. You can now disconnect the Arduino Uno. 
...
The github contains arduino source and kicad pcb design files. You'll find the schematic, the board layout, the bill of materials.

## Note

The kicad pcb hardware design is under Creative Commons - Share Alike license. In the arduino sketch software all files marked "Koen De Vleeschauwer 2019 CC0" I put in the public domain.
